...
function Get-GitRevisionInfo
{
	...
	pushd $dir
	$latestCommit = $(git log --max-count=1)
	popd
	...
	$latestCommit | % -Begin {
		...
		-Process { 
			if ($_ -match '^commit') { $ret | Add-Member NoteProperty Commit ($_ -replace 'commit[\s]*','') }
			if ($_ -match '^Author') { $ret | Add-Member NoteProperty Author ($_ -replace 'Author[\s:]*','') }
			if ($_ -match '^Date')   { $ret | Add-Member NoteProperty Date ($_ -replace 'Date[\s:]*','') }
		} `
		-End {
			Write-Output $info
		}
